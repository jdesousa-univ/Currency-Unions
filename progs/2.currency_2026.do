/****************************************************************************************************
A do-file to determine which "multinational currency" (e.g., US$, Euro, ...) a country is using between 1948 and 2025.
...
Special thanks to Yoto Yotov (Drexel) for encouraging me to keep updating this dataset.
*
* Disclaimer 
...
Source: Glick, Reuven & Rose, Andrew K., 2002. "Does a currency union affect trade? The time-series evidence," European Economic Review, 46(6), pages 1125-1151.
...
6. I updated Glick and Rose's data. 

7. I also added few multinational currencies: 
* The Singapore dollar (SGD), at par with the Brunei dollar (BND) (see CU 11), thanks to Markus Hauschel (Trier U.)
* The New Zealand Dollar (see CU 22)
* The Deutsch Mark Zone (see CU 23)
* The Caribbean Guilder (XCG), replacing the Netherlands Antillean guilder in Curacao and Sint Maarten in 2025 (see CU 2b)
...
10. I have also incorporated the Euro, which was introduced to world financial markets as an accounting currency on January 1, 1999, which was not covered in the first paper of Glick and Rose.
...
12. The sample spans from 1948 to 2025. Thus, all country identifiers are applied retrospectively since 1948, even for territories that gained formal recognition later, such as Aruba (ABW), which was established in 1986.
*/
*
clear all
use tempcty.dta, clear // created with 1.iso3year_2026.do
codebook iso3 year, compact
ta year
*
****************************************************************
* Variables: iso_o iso_d year
* 235 country identifiers, 78 years (1948-2025)
* Observations: 18,330
...
*
*** According to the previous notes, I identify 24 multinational currencies (including 2b):

********************************************************************************
**** 1. the British West Indies dollar (BWI) ****
* known today as Eastern Caribbean Currency Union
* with the Eastern Caribbean Dollar (XCD)
* Source: https://en.wikipedia.org/wiki/Eastern_Caribbean_dollar
********************************************************************************
...
* Added in 2026
replace currency1= "BWI" if (iso3=="VGB") & (year>=1951 & year<1959)
...
* Source: https://en.wikipedia.org/wiki/Netherlands_Antillean_guilder
*********************************************************************************************************

*** 1. Aruba
replace currency1= "ANG" if (iso3=="ABW") & (year<1986)
...
replace currency1= "ANG" if (iso3=="ANT") & (year<2011)
/* Note: The Netherlands Antilles was dissolved on October 10, 2010. 
Curaçao and Sint Maarten became separate constituent countries within the 
Kingdom of the Netherlands, but continued using the Netherlands Antillean 
guilder until the Caribbean guilder was introduced in 2025. As a consequence:

A. if you are handling data for Sint Maarten (SXM) and Curaçao (CUW) 
prior to 2011, it's essential to adjust the code to reflect that they 
shared a common currency during that period.
=> replace currency1= "ANG" if (iso3=="SXM") & (year<2025)

B. ANG co-circulated until July 1, 2025, so you may want to reflect this
replace currency1= "ANG" if (iso3=="SXM") & (year>=2011) & (year<=2025)
*/
*
*** 4. Sint Maarten 
replace currency1= "ANG" if (iso3=="SXM") & (year>=2011) & (year<2025)
...
*** 5. Curacao
replace currency1= "ANG" if (iso3=="CUW") & (year>=2011) & (year<2025)
*
tab currency1
codebook iso3 if currency1=="ANG"

*********************************************************************************************************
**** 2b. The Caribbean Guilder (XCG) ****
*** New in 2025
* The Central Bank of Curacao and Sint Maarten states that the Caribbean guilder became
* legal tender in Curacao and Sint Maarten on March 31, 2025.
* Source: https://cdn.centralbank.cw/media/press_releases_2025/20250330_pb2025_008_caribbean_guilder_legal_tender_en.pdf
*********************************************************************************************************

*** 1. Sint Maarten
replace currency1= "XCG" if (iso3=="SXM") & (year>=2025)

*** 2. Curacao
replace currency1= "XCG" if (iso3=="CUW") & (year>=2025)

tab currency1
codebook iso3 if currency1=="XCG"
...
** 8. Christmas Island
* Added in 2026. Christmas Island was transferred to Australia on October 1, 1958.
replace currency1= "AUD" if (iso3=="CXR") & (year>=1958)
* Source: Australian Government, Christmas Island governance and administration:
...
* See "10. The "Malaya dollar" currency (MYR)"
* Some sources suggest continued use of Malayan currency after 1958, but I have not found a reliable official source.

** 9. Cocos (Keeling) Islands
* Added in 2026. Cocos (Keeling) Islands became an Australian territory on November 23, 1955.
replace currency1= "AUD" if (iso3=="CCK") & (year>=1955)
* Source: Australian Government, Cocos (Keeling) Islands governance and administration:
...
* 5. Namibia (mentioned in footnote 9)
...
* Note: In this file, "MYR" is used as a mnemonic for the historical Malaya-and-British-Borneo dollar area
* before the 1967 currency split.
* It should not be interpreted as the modern Malaysian ringgit.
*********************************************************************************************************
...
* 4. Christmas Island
* Added in 2026
replace currency1= "MYR" if (iso3=="CXR") & (year<1958)
* Christmas Island was administered through Singapore before transfer to Australia in 1958,
* Some sources mention that the Malayan currency was used until 1966, but I don't know how reliable these sources are. (https://stampworldhistory.nl/country-profiles-2/oceania-antarctica/christmas-island/)

* 5. Cocos (Keeling) Islands
* Added in 2026
replace currency1= "MYR" if (iso3=="CCK") & (year<1955)
* Before transfer to Australia in 1955, Cocos (Keeling) Islands was administered through Singapore.
...
*** Note: Here I follow Glick and Rose (2002). Currencies of the following two countries have fluctuated only
*** moderately. Diplomatic relations deteriorated in 2017 but were restored after the Al-Ula declaration in January 2021.
*********************************************************************************************************
...
** 32. Pitcairn Islands (British Overseas Territory)
* Added in 2026
replace currency1= "GBP" if (iso3=="PCN") & (year<1967)
* Pitcairn now uses the New Zealand dollar; NZD replaced the New Zealand pound in 1967.
...
``The Liberian Dollar is the official currency of the Republic of Liberia. 
 ...
 alongside the Liberian Dollar currency. */
* 
** 10. Panama 
replace currency1= "USD" if (iso3=="PAN") 

** 11. Ecuador 
* Correction in 2026: Ecuador dollarization should include year 2000
* source: https://en.wikipedia.org/wiki/Currency_of_Ecuador
replace currency1= "USD" if (iso3== "ECU") & (year>=2000)

** 12. El Salvador
replace currency1= "USD" if (iso3== "SLV") & (year>=2001)
/* From the Central Bank of El Salvador; ``The US Dollar becomes the unit of account for the Financial System. All the financial System's operations and accounting will be denominated in US Dollars. All deposits, credits, pensions and other operations of the financial system will be automatically converted into US dollars on January 1st 2001.''
https://web.archive.org/web/20070708094105/http://www.bcr.gob.sv/ingles/integracion/ley.html
*/
  
** 13. East Timor
* Correction in 2026: (United Nations Transitional Administration in East Timor) UNTAET Regulation 2000/7 established the US dollar as East Timor’s official currency in January 2000. "This arrangement has been maintained since Timor-Leste's independence in May 2002."
* Source: https://www.imf.org/-/media/files/publications/selected-issues-papers/2024/english/sipea2024042.pdf
replace currency1= "USD" if (iso3=="TLS") & (year>=2000)
...
* If you prefer to consider Argentina dollarized during this period:
* replace currency1= "USD" if (iso3=="ARG") & (year>=1995 & year<=2001)

** 19. Puerto Rico
* Added in 2026: As I have an ISO3 code for Puerto Rico (PRI).
replace currency1= "USD" if (iso3=="PRI")
* Puerto Rico is a U.S. territory and uses the U.S. dollar.
* Source: https://en.wikipedia.org/wiki/Puerto_Rico

** 20. British Virgin Islands
* Added in 2026 
replace currency1= "USD" if (iso3=="VGB") & (year>=1959)
* Source: Government of the Virgin Islands states that the BVI adopted the US dollar as its currency in 1959.
* Government of the Virgin Islands: https://www.bvi.gov.vg/media-centre/changing-usd-currency-not-option-bvi

** 21. Marshall Islands
* Added in 2026.  
replace currency1= "USD" if (iso3=="MHL")
* Source: https://www.state.gov/reports/2024-investment-climate-statements/marshall-islands
...
** 33. Bulgaria
* Added in 2026. However, the generated panel currently stops in 2025, so that code never bites unless you update "1.iso3year_2026.do" with global lstyr "2026"  instead of global lstyr "2025"
*
replace currency1= "EUR" if (iso3=="BGR") & (year>=2026)	
* The Council of the EU adopted the final acts for Bulgaria to introduce the euro on January 1, 2026, and the European Commission lists Bulgaria as having adopted the euro on January 1, 2026
*Source: https://economy-finance.ec.europa.eu/euro/eu-countries-and-euro/bulgaria-and-euro_en
...
* Added in 2026
replace currency2= "NZD" if (iso3=="PCN") & (year>=1967)
* Pitcairn Islands uses the New Zealand dollar as official currency; NZD was introduced in 1967.
* Source: https://en.wikipedia.org/wiki/New_Zealand_dollar
